...
export function describeTelegramError(err: unknown): string {
  if (err instanceof TelegramApiError) {
    if (/not enough rights/i.test(err.description)) {
      return "The bot doesn't have permission to create invite links for this channel. Open the channel's Administrators list, edit the bot's rights, and turn on \"Invite Users via Link\" — then try again.";
    }
    if (/chat not found/i.test(err.description)) {
      return "This channel isn't reachable anymore — the bot may have been removed. Reconnect it from Settings.";
    }
    return `Telegram rejected this: ${err.description}`;
  }
  return "Something went wrong talking to Telegram. Try again in a moment.";
}
