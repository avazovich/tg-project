...
export default function PlacementPreview({ t }: { t: PlacementPreviewText }) {
  return (
    <div className="rounded-[20px] border border-[#e7e3e3] bg-white p-5 shadow-[0_30px_70px_0_rgba(28,31,46,0.12)] md:p-6">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <div className="text-xs text-[#8e8f8f]">{t.dateRange}</div>
      </div>

      <div className="mt-4 flex flex-wrap gap-5">
        <Segment label={t.topSlot} value="119" share={71} color="#3629b7" />
        <Segment label={t.restOfFeed} value="38" share={23} color="#8b7fe8" />
        <Segment label={t.afterWindow} value="10" share={6} color="#c9c4e8" />
      </div>
